...
To keep the business logic layer insulated from these changes, we introduced a small internal SDK. It provided a stable interface for the business layer and delegated calls to the appropriate provider connector under the hood. This SDK encapsulated provider selection logic and acted as a boundary between the modularized integration code and the core application. It also helped us maintain a consistent developer experience during the transition.
